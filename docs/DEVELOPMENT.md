...
Complete guide for developers working on ContextDo.
...
Output: `contextdo-vscode-1.0.0.vsix`
...
# In Cursor
cursor --install-extension contextdo-vscode-1.0.0.vsix --force
```
...
2. **Read in code**:
```typescript
const config = vscode.workspace.getConfiguration('contextdo');
...
vscode.workspace.onDidChangeConfiguration(e => {
    if (e.affectsConfiguration('contextdo.mySetting')) {
        // Handle change
    }
});
```
...
**Solutions**:
1. Use full command name: "ContextDo: ..."
